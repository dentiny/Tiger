...
(*Finally the semantic analyzer calls procEntryExit, which has the side effect of remembering a PROC fragment.*)
fun procEntryExit {level=Level{parent, frame, levelRef}, body = bodyExp} =
  let
    val body = unEx(bodyExp)
    val proc = F.PROC{body=T.MOVE(T.TEMP(F.RV), unEx(bodyExp)),frame=frame}
  in
    fragList := !fragList@[proc]
  end
  | procEntryExit {level=BaseLevel, body} = bugErr "cannot exit BaseLevel!"
  ...
  in
    ...
    | NONE =>
      ...
      (
        ...
        fragList := F.STRING(strLabel, v)::(!fragList);
        Ex(T.NAME(strLabel))
      )
      end
  end
...
(*
  constant folding:
    for two constant integers, compare their value
    for two string literals, compare their label
*)
fun transRelop (A.EqOp,  Ex(T.NAME(lab1)), Ex(T.NAME(lab2)), Types.STRING) = transBool(getStringLiteral(lab1) =  getStringLiteral(lab2))
  ...
  | transRelop (A.EqOp,  left, right, Types.STRING) = Ex(F.externalCall("stringEqual", [unEx(left), unEx(right)]))
...
fun transFor (lowExp, highExp, bodyExp, breakLabel) =
  let
    val lo = unEx(lowExp)
    val hi = unEx(highExp)
    val body = unNx(bodyExp)
    val forVar = Temp.newtemp()
    val hiReg = Temp.newtemp()
    val t = Temp.newlabel()
  in
    Nx(seq([
      T.MOVE(T.TEMP(forVar), unEx(lowExp)),
      T.MOVE(T.TEMP(hiReg), hi),
      T.CJUMP(T.LE, T.TEMP(forVar), T.TEMP(hiReg), t, breakLabel), (* compare only once at the beginning of loop *)
      T.LABEL(t),
      body,
      T.MOVE(T.TEMP(forVar), T.BINOP(T.PLUS, T.TEMP(forVar), T.CONST(1))),
      T.CJUMP(T.LT, T.TEMP(forVar), T.TEMP(hiReg), t, breakLabel),
      T.LABEL(breakLabel)
    ]))
  end
...
fun transMkArray (sizeExp, initExp) =
  ...
  in
    Ex(T.ESEQ(seq([
      T.MOVE(T.TEMP(arrAddr), F.externalCall("initArray", [sizeArg, init])),
...
(*
  1. Allocate memory via malloc(len)
  2. Assign field value to recordAddr[offset]
*)
  ...
  let
    ...
    fun assignMem (exp, (idx, l)) =
      let
        ...
        val assignMemVal' = assignMemVal(getMemAddr', unEx(exp))
      in
        (idx + 1, l@[assignMemVal'])
      end
    val (_, assignInstru) = foldl assignMem (0, []) fieldList
    val allocInstru = T.MOVE(T.TEMP(addr), F.externalCall("malloc", [T.CONST(fieldLen * F.wordSize)]))
  in
    Ex(T.ESEQ(
...
fun transSubscript (var, index) =
  ...
  in
    Ex(T.ESEQ(seq([
      ...
      T.LABEL(failLabel),
      T.EXP(F.externalCall("exit", [T.CONST(1)])), (* exit(errno); exits the program abnormally, meanwhile errno indicates the specific error *)
      T.LABEL(suc2Label)
      ]),
