...
int stringNotEqual(struct string *s, struct string *t) {
  return 1 - stringEqual(s, t);
}

int stringLessThan(struct string *s1, struct string *s2) {
  int idx1 = 0;
  int idx2 = 0;
  while (idx1 < s1->length && idx2 < s2->length) {
    if (s1->chars[idx1] < s2->chars[idx2]) {
      return 1;
    }
    ++idx1;
    ++idx2;
  }
  return idx1 == s1->length && idx2 != s2->length ? 1 : 0;
}

int stringLessEqual(struct string *s1, struct string *s2) {
  int idx1 = 0;
  int idx2 = 0;
  while (idx1 < s1->length && idx2 < s2->length) {
    if (s1->chars[idx1] < s2->chars[idx2]) {
  ...
  return idx1 == s1->length ? 1 : 0;
}

int stringGreaterThan(struct string *s1, struct string *s2) {
  int idx1 = 0;
  int idx2 = 0;
  while (idx1 < s1->length && idx2 < s2->length) {
    if (s1->chars[idx1] > s2->chars[idx2]) {
  ...
  return idx2 == s2->length && idx1 != s1->length ? 1 : 0;
}

int stringGreaterEqual(struct string *s1, struct string *s2) {
  int idx1 = 0;
  int idx2 = 0;
  while (idx1 < s1->length && idx2 < s2->length) {
    if (s1->chars[idx1] > s2->chars[idx2]) {
  ...
  return idx2 == s2->length ? 1 : 0;
}

enum err { OK = 0, OUT_OF_BOUND };
void exitProg(int errno) {
  switch (errno)
  {
  case OK:
    exit(EXIT_SUCCESS);
    ...
    break;
  case OUT_OF_BOUND:
    fprintf(stderr, "Array out of bound");
    exit(EXIT_FAILURE);
    assert(0);
    break;
  default:
