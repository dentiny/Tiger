...
fun procEntryExit {level=Level{parent, frame, levelRef}, body = bodyExp} =
  let
    ...
    val proc = F.PROC{body=T.MOVE(T.TEMP(F.V0), unEx(bodyExp)),frame=frame}
  in
    fragList := proc::(!fragList)
  end
...
fun transSubscript (var, index) =
  ...
  in
    Ex(T.ESEQ(seq([
      ...
      T.LABEL(failLabel),
      T.EXP(F.externalCall("exitProg", [T.CONST(1)])), (* exit(errno); exits the program abnormally, meanwhile errno indicates the specific error *)
      T.LABEL(suc2Label)
      ]),
