...
struct
          ...
          *)
          ...
          | trexp (A.ForExp{var, escape, lo, hi, body, pos}) =
            let
              ...
              val tempVenv = S.enter(venv, var, E.VarEntry{access = Tr.allocLocal(curLevel)(!escape), ty = T.INT})
            ...
            (
              ...
              {exp = Tr.transFor(loExp, hiExp, bodyExp, breakLabel), ty = T.UNIT}
            )
            end
                ...
                *)
                ...
                fun checkBody({name, params, body, pos, result}, venv) =
                    let
                      ...
                      val _ = Tr.procEntryExit{level=funLevel, body=bodyExp}
                   in
                       venv
                   end
    ...
    fun transProg(exp: A.exp) : Translate.frag list =
      let
        val startLabel = Temp.namedlabel("tigermain")
        ...
        val () = Tr.procEntryExit({level = startLevel, body = exp'})
      in
        Tr.getResult()
      end
